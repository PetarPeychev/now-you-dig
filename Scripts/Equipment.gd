...
func _ready():
	for slot in get_tree().get_nodes_in_group("EquipmentSlots"):
...
func slot_gui_input(event : InputEvent, slot : ItemSlot):
	if event is InputEventMouseButton:
		if event.button_index == BUTTON_LEFT && event.pressed:
			if GameManager.held_item && GameManager.held_item.item_resource.slot_type == slot.slot_type:
				if slot.item:
			...
			elif not GameManager.held_item && slot.item:
